...
<script>
	 ...
	 function showName(obj, id) {
		 var name = $(obj).text();
		 $("#dropdownMenuGetValue").text(name);
		 $("#status").val(id);
	 }
	...
	<div class="jumbotron" style="padding-top: 20px; padding-bottom: 25px">
	 ...
	 <div class="container" >
			       ...
			       <div class="row">
			          ...
			          <div class="col-md-7">
			                      <form action="<%=path %>/OrderServlet" class="form-inline" id="formForOrder">
									  <input type="hidden" value="findOrdersByPage" name="method">
														...
														<ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
																...
																<li>
																  <c:forEach items="${orderStatus}" var="status">
																	  <a  class="btn" name="io" onclick="showName(this,${status.sid})">${status.statusName}</a>
																  </c:forEach>
...
<%--							</a></td>--%>
							...
							<td><a class="glyphicon glyphicon-trash" color="#5BC0DE"
								...
								href="<%=path%>/OrderServlet?method=deleteOrderById&ordersid=${orders.id}"
	...
	<div class="container">
		<nav aria-label="..." class="navbar-right" style="margin-right: 15px">
			<ul class="pagination">
				<li><a class=" btn-info"
					href="<%=path %>/OrderServlet?method=findOrdersByPage&pageNo=1" aria-label="Previous">
						<span aria-hidden="true">首页 </span>
				</a></li>
				<c:if test="${page.pageNo>1}">
					<li><a class=" btn-info"
						   href="<%=path %>/OrderServlet?method=findOrdersByPage&pageNo=${page.pageNo-1}&username=${username}&status=${status}"
				...
				<c:if test="${page.pageNo<page.pageCount}">
					...
					<li><a class=" btn-info"
						   href="<%=path %>/OrderServlet?method=findOrdersByPage&pageNo=${page.pageCount}"
						   aria-label="Previous"> <span aria-hidden="true">尾页 </span></a></li>

				</c:if>
				<c:if test="${page.pageNo>=page.pageCount}">
					<li><a class=" btn-info"
						   href="<%=path %>/OrderServlet?method=findOrdersByPage&pageNo=${page.pageNo}">${page.pageNo}</a></li>
					<li class="disabled"><a href="#" aria-label="Previous">»<span
							class="sr-only">(current) </span></a></li>
